...
def sentiment_file_exists(layanan: str) -> bool:
    """Cek apakah sumber sentimen CSV, Excel, atau ZIP tersedia di disk."""
    try:
        return _resolve_sentiment_source(layanan) is not None
    except Exception:
        return False
...
def get_data_source_label(layanan: str) -> str:
    ...
    try:
        # Khusus IndiBiz, keberadaan file mentah saja belum cukup. File baru
        # disebut data real jika sudah memiliki output prediksi Fase 11 yang valid.
        if str(layanan).strip() == "IndiBiz":
