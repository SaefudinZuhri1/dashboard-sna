    ...
    try:
        st.markdown(
            ...
            <style>
                ...
                .topic-v8-stat-label { color: #AAAAAA; font-size: 0.74rem; font-weight: 650; }
                .topic-v8-stat-value {
                    color: #E53935;
                    font-family: 'Plus Jakarta Sans', 'Inter', sans-serif;
                    font-size: 1.7rem;
                ...
                .topic-v8-topic-meta { color: #AAAAAA; font-size: 0.74rem; margin-top: 0.25rem; }
...
def _load_enriched_topic_data(layanan: str, file_signature: str) -> pd.DataFrame:
    """Klasifikasikan topik satu kali per versi file lalu simpan ke cache disk."""
    del file_signature  # Menjadi cache key untuk mendeteksi perubahan file sumber.
    ...
    result["platform"] = result["platform"].map(_normalize_platform)
    ...
    # IndiBiz memakai satu model LDA global untuk membentuk lima topik total.
    # Layanan lain tetap memakai klasifikasi kata kunci yang sudah stabil.
    if layanan == "IndiBiz":
        return result
...
def _plotly_chart(fig: go.Figure, key: str) -> None:
    """Render Plotly responsif dengan fallback kompatibilitas Streamlit."""
    try:
        st.plotly_chart(
            fig,
            use_container_width=True,
...
def render_topic_analysis() -> None:
    ...
    try:
        ...
        file_signature = get_sentiment_file_signature(layanan)
        ...
        if df.empty:
            st.warning(
                "Tidak ada komentar pada kombinasi filter ini. Pilih platform atau sentimen lain."
            )
