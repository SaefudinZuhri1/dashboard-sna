...
def _render_pagerank_overview(node_df: pd.DataFrame, service: str) -> None:
    ...
    try:
        ...
        st.plotly_chart(fig, use_container_width=True, key=f"sna_pagerank_{service.lower()}")
    ...
    try:
        ...
        with st.container(border=True):
            ...
            with st.form("sna_v9_filter_form", clear_on_submit=False):
                col_service, col_platform, col_limit = st.columns([1, 1, 1])
                with col_service:
        ...
        return selected_service, PLATFORM_OPTIONS[selected_platform_label], int(node_limit), bool(submitted)
    except Exception as exc:
...
def render_sna() -> None:
    ...
    try:
        ...
        _render_hero(active_service, sna_file_exists(active_service))
